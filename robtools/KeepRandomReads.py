...
def keeprandomreads(samples, count, paired, threads, input_suffix, output_suffix, index):
...
def keeprandomreads_sample(sample, count=10000000, paired=True, threads=None, input_suffix='', output_suffix='-random'):
    ...
    count_read_callback = (lambda r: (
                                             r.is_read1 or not paired) and is_primary(r))
    with pysam.AlignmentFile(sort_bam, 'rb') as inbam:
        header = inbam.header
        in_count = inbam.count(until_eof=True,
                               read_callback=count_read_callback)
    ...
    with pysam.AlignmentFile(sort_bam, 'rb') as inbam, pysam.AlignmentFile(filter_bam, 'wb', header=header) as outbam:
        ...
        for aln in inbam.fetch(until_eof=True):
            ...
            i = i + 1
    logging.debug(
        f"sample = {sample}, count = {count}, in_count = {in_count}, len indexes = {len(indexes)},"
