...
def test_robtools_download(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['download', '--samples', samples, '--slow', '--threads', threads, '--mem', mem, '--index', index])
...
def test_robtools_filterbam(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['filterbam', '--samples', samples, '--unpaired', '--threads', threads, '--index', index])
    assert result.exit_code == 0
...
def test_robtools_genomecov(testdir, mock_testclass):
    ...
    assert result.exit_code == 0
    GenomeCoverage.genome_coverage_samples.assert_called_once_with(samples, sizes, scale, strand, input_suffix, output_suffix, None, None, 1, index, ('-5',))
...
def test_robtools_intersect(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['intersect', '--samples', samples, '--annotations', annotations, '--index', index])
    assert result.exit_code == 0
...
def test_robtools_slowsplit(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['slowsplit', '--samples', samples, '--binLength', binlength, '--binMinLength', binminlength, '--binMaxLength', binmaxlength, '--index', index])
    logging.warning(result.output)
    assert result.exit_code == 0
...
def test_robtools_split(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['split', '--samples', samples, '--binLength', binlength, '--binMinLength', binminlength, '--binMaxLength', binmaxlength, '--index', index])
    logging.warning(result.output)
    assert result.exit_code == 0
...
def test_robtools_vap(testdir, mock_testclass):
    ...
    runner = CliRunner()
    result = runner.invoke(robtools.robtools, ['vap', '--samples', samples, '--parameters', parameters, '--index', index])
    logging.warning(result.output)
    assert result.exit_code == 0
